...
describe command('dig -p 53 chaos txt version.bind @127.0.0.1 +short') do
  its('stdout.chomp') { should match(Regexp.new(/"PowerDNS Recursor 5\.2\.\d/)) }
end
...
describe command('rec_control --config-name=server_02 --socket-dir=/run/pdns-recursor-server_02 reload-zones') do
